...
class Conv2D:
    ...
    def initialize_weights(self):
        ...
        if self.initialize_method == "random":
            ...
            np.sqrt(1 / (self.in_channels * self.kernel_size[0] * self.kernel_size[1]))
        elif self.initialize_method == "he":
            return np.random.randn(self.kernel_size[0], self.kernel_size[1], self.in_channels, self.out_channels) * \
            np.sqrt(2 / (self.in_channels * self.kernel_size[0] * self.kernel_size[1]))       
        else:
            ...
            for h in range(H):
                ...
                for w in range(W):
                    ...
                    for c in range(C):
                        ...
                        Z[i, h, w, c] = self.single_step_convolve(a_slice_prev, W[h, w, c], b[h, w, c])
        return Z
    ...
    def backward(self, dZ, A_prev):
        ...
        dW = np.zeros(W.shape)        # hint: same shape as W
            ...
            for h in range(H):
                for w in range(W):
                    for c in range(C):
                        ...
                        da_prev_pad[h_start:h_end, w_start:w_end, :] += np.multiply(W[..., c], dZ[i, h, w, c]) # hint: use element-wise multiplication of dZ and W
